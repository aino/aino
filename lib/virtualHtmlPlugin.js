...
const getTemplateContext = async (url, prefetched) => {
...
export default function virtualHtmlPlugin(prefetched) {
  return {
    name: 'virtual-html',
    configurePreviewServer(server) {
      server.middlewares.use(async (req, res, next) => {
        try {
          ...
          let templateContext = await getTemplateContext(req.url, prefetched)
          if (!templateContext) {
            templateContext = await getTemplateContext(
              getLocalized404(req.url),
              prefetched
            )
            if (!templateContext) {
              return next()
    ...
    configureServer(server) {
      server.middlewares.use(async (req, res, next) => {
        try {
          ...
          let templateContext = await getTemplateContext(url, prefetched)
          if (!templateContext) {
            templateContext = await getTemplateContext(
              getLocalized404(url),
              prefetched
