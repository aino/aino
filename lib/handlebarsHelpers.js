...
export default function handlebarsHelpers(Handlebars) {
  ...
  Handlebars.registerHelper('column', function (context) {
    ...
    const content = context.fn({
      ...this,
      title: context.data.root.title,
    })
    let styles = []
    if (top) {
  ...
  Handlebars.registerHelper('img', (context) => {
    let { colWidth = 2, url, width, height, sizes, alt } = context.hash
    if (!url) {
      console.log('No URL provided for image helper', context)
    }
    if (colWidth && !sizes) {
      const fraction = colWidth / 8
      sizes = `(max-width: 768px) 75vw, ${fraction * 90}vw`
    }
    ...
    const srcSet = imageSizes
      ...
      .join(', ')
    return new Handlebars.SafeString(
      `src="${src}" srcSet="${srcSet}"${props} alt="${alt || 'Aino Agency'}"`
    )
  })
}
